...
Renderer::Renderer(Platform::Device* pDevice)
    : Platform::BaseRenderer(pDevice, 2, 32, { sizeof(Matrix4f), sizeof(Lights) }, 1 + ShadowSplits)
    , CameraControlEuler()
    , m_pTextDraw(nullptr)
    , m_fpsCount(0)
    , m_prevFPS(0.0)
...
bool Renderer::Init(HWND hWnd)
{
    ...
    if (res)
    {
        ...
        if (res)
        {
            ...
            res = m_pModelLoader->Init(this, modelFiles, HDRFormat, HDRFormat, false, false);
        }
        ...
        if (res)
        {
            ...
            m_pPlayerModelLoader->Init(this, modelFiles, HDRFormat, HDRFormat, false, false);
        }
    }
                ...
                && GetDevice()->TransitResourceState(GetCurrentCommandList(), m_bloomRT[0].pResource, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_RENDER_TARGET))
            {
                ...
                if (m_sceneParams.showMenu)
                {
                    ...
                    else
                    {
                        ...
                        m_sceneParams.pModel = m_pPlayerModelLoader->GetModel(m_sceneParams.modelIdx);
                    }
...
void Renderer::MeasureLuminance()
{
    ...
    for (int i = 0; i < sizeof(steps)/sizeof(MeasureLuminanceStep); i++)
    {
        ...
        if (i == 0)
        {
            res = GetDevice()->TransitResourceState(GetCurrentCommandList(), m_hdrRT.pResource, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
        }
        else if (i == 1)
        {
            res = GetDevice()->TransitResourceState(GetCurrentCommandList(), m_lumTexture0.pResource, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
        }
        ...
        if (res)
        {
            if (i == 0)
            {
                res = GetDevice()->TransitResourceState(GetCurrentCommandList(), m_hdrRT.pResource, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
            }
            else if (i == 1)
            {
                res = GetDevice()->TransitResourceState(GetCurrentCommandList(), m_lumTexture0.pResource, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
            }
        }
        assert(res);
    }
...
bool Renderer::CreateHDRTexture()
{
    ...
    if (res)
    {
        ...
        {
            m_hdrRT.pResource->SetName(L"HDR RenderTarget");
        }
    ...
    if (res)
    {
        ...
        if (res && GetDevice()->IsDebug())
        {
            m_lumTexture0.pResource->SetName(L"LuminanceTexture0");
        }
    }
    if (res)
    {
        ...
        if (res && GetDevice()->IsDebug())
        {
            m_lumTexture1.pResource->SetName(L"LuminanceTexture1");
        }
    }
    ...
    if (res)
    {
        ...
        for (int i = 0; i < 2 && res; i++)
        {
            ...
            if (res && GetDevice()->IsDebug())
            {
                m_bloomRT[i].pResource->SetName(i == 0 ? L"Bloom 0" : L"Bloom 1");
            }
...
bool Renderer::CreateComputePipeline()
{
    ...
    if (res)
    {
        ...
        if (SUCCEEDED(hr) && GetDevice()->IsDebug())
        {
            m_pLuminancePSO->SetName(L"Luminance.hlsl");
        }
    ...
    if (res)
    {
        ...
        if (SUCCEEDED(hr) && GetDevice()->IsDebug())
        {
            m_pLuminanceFinalPSO->SetName(L"LuminanceFinal.hlsl");
        }
    ...
    if (res)
    {
        ...
        if (SUCCEEDED(hr) && GetDevice()->IsDebug())
        {
            m_pComputeBlurHorzPSO->SetName(L"Bloom.hlsl");
        }
    ...
    if (res)
    {
        ...
        if (SUCCEEDED(hr) && GetDevice()->IsDebug())
        {
            m_pComputeBlurVertPSO->SetName(L"Bloom.hlsl");
        }
...
void Renderer::PrepareColorPass()
{
    UINT8* dynCBData[2] = {};
    BeginRenderParams beginParams = {
        {BackColor.x, BackColor.y, BackColor.z, BackColor.w},
    ...
    Point4f camPos = GetCamera()->CalcPos();
    Point3f right, up, dir;
    GetCamera()->CalcDirection(right, up, dir);
    pCommonCB->cameraWorldPosNear = Point4f{ camPos.x, camPos.y, camPos.z, GetCamera()->GetNear()};
    pCommonCB->cameraWorldDirFar = Point4f{ dir.x, dir.y, dir.z, GetCamera()->GetFar() };
