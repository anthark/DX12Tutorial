...
PSOut PS(VSOut input)
{
	...
	switch (renderMode)
	{
		...
		default:
			pbrColor = CalcPBRColor_KHRSpecGloss(input.worldPos, d.n, d.v, roughness, Cdiff.xyz, F0.xyz, input.pos.xy);// * GetTint(input.worldPos);
			break;
	}
...
PSOut PS(VSOut input)
{
	...
	switch (renderMode)
	{
		...
		default:
			pbrColor = CalcPBRColor(input.worldPos, d.n, d.v, roughness, metalness, dielectricF0, F0.xyz, input.pos.xy);// * GetTint(input.worldPos);;
			break;
	}
