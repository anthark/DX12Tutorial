...
float4 PS(VSOut input) : SV_TARGET
{
    ...
    for(uint i = 0u; i < SAMPLE_COUNT; ++i)
    {
        ...
        if(ndotl > 0.0)
        {
            prefilteredColor += ColorTexture.SampleLevel(Sampler, L, mipLevel).rgb * ndotl;
            totalWeight      += ndotl;
        }
    }

    /*float3 up    = float3(0.0, 1.0, 0.0);
    float3 right = cross(up, norm);
    up           = cross(norm, right);
    ...
    for(float phi = 0.0; phi < 2.0 * PI; phi += sampleDelta)
    {
        for(float theta = 0.0; theta < roughness * roughness * 0.5 * PI + 0.001; theta += sampleDelta)
        {
            // spherical to cartesian (in tangent space)
            float3 tangentSample = float3(sin(theta) * cos(phi),  sin(theta) * sin(phi), cos(theta));
            // tangent space to world
            float3 H = tangentSample.x * right + tangentSample.y * up + tangentSample.z * norm;
            float3 L  = normalize(2.0 * dot(view, H) * H - view);

            float ndotl = max(dot(norm, L), 0.0);
            if(ndotl > 0.0)
            {
                prefilteredColor += ColorTexture.Sample(Sampler, L).rgb * ndotl;
                totalWeight      += ndotl;
            }
        }
    }*/
